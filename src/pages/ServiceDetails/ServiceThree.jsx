  ...
  return (
    <div>
      ...
      <div className="px-[20px] py-[10px] md:px-[100px] md:py-[30px]">
        ...
        <span className="underline font-semibold text-md lg:text-lg">Below our Accounting Services:</span>
                ...
                {dataArr.map((data, idx) => (
                    <div key={idx} onClick={() => handleToggle(idx)} className="flex items-center">
                        ...
                        <div className="relative h-[2px] w-5 md:w-10 bg-[#0149AB]">
                            ...
                            <span
                                className={`absolute -right-2 -top-[5px] z-40 h-3 w-3 rounded-full border-2 ${isOpen === idx ? 'border-[#0149AB] bg-white delay-100' : 'border-transparent'}`}
                            ></span>
                        </div>
                        ...
                        <div className="text-center">
                            <div className="relative md:max-w-[750px] lg:max-w-[850px] border-t-[12px] border-[#0149AB] bg-sky-50 p-3 shadow-md">
                                ...
                                <h1 className="select-none text-lg text-[#0149AB]">{data.title}</h1>
                            </div>
                            <div className={`grid overflow-hidden text-slate-600 transition-all duration-300 ease-in-out ${isOpen === idx ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                                <div className="overflow-hidden">
                                    <div className="max-w-[280px] md:max-w-[550px] lg:max-w-[950px] bg-[#0149AB] p-6 text-sm text-white">{data.description}</div>
                                </div>
                            </div>
                        </div>
                    </div>
